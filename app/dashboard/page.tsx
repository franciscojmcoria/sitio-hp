...
function Barra({ value, max, cor }: { value:number; max:number; cor:string }) {
...
export default function Dashboard() {
  ...
  const [form, setForm] = useState({
    data:'', descricao:'', categoria:'Alvenaria', fornecedor:'',
    valor:'', forma_pagamento:'PIX', documento:'', obs:''
  })
  ...
  const uploadArquivo = async (file: File): Promise<string|null> => {
    ...
    setUploadPct(30)
    const { data, error } = await sb.storage.from('comprovantes').upload(nome, file)
    setUploadPct(80)
    if (error || !data) { setUploadPct(0); return null }
    const { data: { publicUrl } } = sb.storage.from('comprovantes').getPublicUrl(data.path)
    setUploadPct(100); setTimeout(() => setUploadPct(0), 1000)
    return publicUrl
  }
  ...
  const adicionar = async () => {
    ...
    if (arquivo) comprovante_url = await uploadArquivo(arquivo)
    await sb.from('lancamentos').insert({
      ...form, valor:parseFloat(form.valor),
      documento:form.documento||null, obs:form.obs||null,
      comprovante_url
    })
    setSaving(false)
  ...
  const tabBtn = (t: string): React.CSSProperties => ({
    ...
    fontWeight:tab===t?700:400, background:tab===t?S.gold:'transparent',
    color:tab===t?'#000':S.muted, border:'none', transition:'all .2s'
  })
        ...
        {tab==='dash' && (
          <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:16 }}>
              {[
                {l:'TOTAL PREVISTO',  v:fmt(totalPrev), c:'#4FC3F7', s:'orçamento base'},
                {l:'TOTAL REALIZADO', v:fmt(totalReal), c:S.gold,    s:`${lance.length} pagamentos`},
                {l:'SALDO DISPONÍVEL',v:fmt(saldo),     c:saldo>=0?'#22C55E':'#EF4444', s:saldo>=0?'dentro do orçamento':'⚠️ estouro'},
                {l:'% EXECUTADO',     v:`${pct.toFixed(1)}%`, c:'#A78BFA', s:'do total previsto'}
              ].map(({l,v,c,s})=>(
            ...
            <div style={card}>
              ...
              <Barra value={totalReal} max={totalPrev} cor={S.gold}/>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1.4fr', gap:16 }}>
              ...
              <div style={card}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                  {lance.slice(0,6).map(l=>(<div key={l.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', paddingBottom:10, borderBottom:`1px solid ${S.bord}` }}><div><div style={{ fontSize:12, fontWeight:700, marginBottom:3, display:'flex', alignItems:'center', gap:6 }}>{l.descricao}{l.comprovante_url&&<a href={l.comprovante_url} target="_blank" rel="noreferrer" style={{ fontSize:14, textDecoration:'none' }} title="Ver comprovante">📎</a>}</div><div style={{ display:'flex', gap:8, alignItems:'center' }}><span style={{ fontSize:10, color:S.muted }}>{fmtD(l.data)}</span><Badge cat={l.categoria} cor={corMap[l.categoria]||S.gold}/></div></div><div style={{ textAlign:'right' }}><div style={{ fontSize:14, fontWeight:800, color:S.gold }}>{fmt(l.valor)}</div><div style={{ fontSize:10, color:S.muted }}>{l.forma_pagamento}</div></div></div>))}
                </div>
              </div>
            </div>
            <div style={card}>
        ...
        {tab==='lanc' && (
          <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
            <div style={{ display:'flex', gap:12, alignItems:'center', flexWrap:'wrap' }}>
              ...
              <select value={filtCat} onChange={e=>setFiltCat(e.target.value)} style={{ ...inp, width:'auto', cursor:'pointer' }}>
            ...
            {showForm && (
              <div style={{ ...card, border:`1px solid ${S.gold}55` }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))', gap:12, marginBottom:12 }}>
                  {[
                    {l:'DATA *',       k:'data',           t:'date'},
                    {l:'VALOR (R$) *', k:'valor',          t:'number'},
                    {l:'FORMA',        k:'forma_pagamento', t:'select'},
                    {l:'CATEGORIA',    k:'categoria',       t:'select2'},
                    {l:'DESCRIÇÃO *',  k:'descricao',       t:'text'},
                    {l:'FORNECEDOR *', k:'fornecedor',      t:'text'},
                    {l:'DOCUMENTO',    k:'documento',       t:'text'},
                    {l:'OBSERVAÇÃO',   k:'obs',             t:'text'},
                  ].map(({l,k,t})=>(
                    <div key={k}>
                      <label style={{ display:'block', fontSize:10, color:S.muted, letterSpacing:2, marginBottom:4 }}>{l}</label>
                      {t==='select'  ? <select value={(form as any)[k]} onChange={e=>setForm({...form,[k]:e.target.value})} style={{ ...inp, cursor:'pointer' }}>{FORMAS.map(f=><option key={f}>{f}</option>)}</select>
                       :t==='select2'? <select value={(form as any)[k]} onChange={e=>setForm({...form,[k]:e.target.value})} style={{ ...inp, cursor:'pointer' }}>{CATS.map(c=><option key={c}>{c}</option>)}</select>
                       :<input type={t} value={(form as any)[k]} onChange={e=>setForm({...form,[k]:e.target.value})} style={inp} placeholder={k==='valor'?'0,00':undefined}/>}
                    </div>
                  ))}
                </div>
                ...
                <div style={{ marginBottom:16 }}>
                  <label style={{ display:'block', fontSize:10, color:S.muted, letterSpacing:2, marginBottom:6 }}>📎 COMPROVANTE (foto/PDF)</label>
                  <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                    ...
                    />
                    {arquivo && <span style={{ fontSize:11, color:'#22C55E', whiteSpace:'nowrap' }}>✅ {arquivo.name}</span>}
                  </div>
                  {uploadPct>0 && (
                    <div style={{ marginTop:8 }}>
                      <div style={{ background:S.surf2, borderRadius:4, height:4, overflow:'hidden' }}>
                        <div style={{ width:`${uploadPct}%`, height:'100%', background:S.gold, transition:'width .3s' }}/>
                      </div>
                      <div style={{ fontSize:10, color:S.muted, marginTop:4 }}>Enviando comprovante... {uploadPct}%</div>
                    </div>
                  )}
                </div>

                <div style={{ display:'flex', gap:10 }}>
                  <button onClick={adicionar} disabled={saving} style={{ background:saving?'#A07818':S.gold, color:'#000', border:'none', borderRadius:8, padding:'9px 22px', fontWeight:800, cursor:saving?'not-allowed':'pointer' }}>
                    {saving?'Salvando...':'✅ Salvar'}
                  </button>
                  <button onClick={()=>{ setShowForm(false); setArquivo(null); if(fileRef.current) fileRef.current.value='' }} style={{ background:'transparent', color:S.muted, border:`1px solid ${S.bord}`, borderRadius:8, padding:'9px 18px', cursor:'pointer' }}>Cancelar</button>
                </div>
              </div>
            )}
            ...
            <div style={card}>
              <div style={{ fontSize:11, color:S.muted, marginBottom:12 }}>
                {filtered.length} lançamento(s) · <strong style={{ color:S.gold }}>{fmt(filtered.reduce((a,b)=>a+b.valor,0))}</strong>
              </div>
              <div style={{ overflowX:'auto' }}>
                <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
                  <thead><tr style={{ borderBottom:`1px solid ${S.bord}` }}>
                    {['DATA','DESCRIÇÃO','CATEGORIA','FORNECEDOR','FORMA','COMP.','VALOR',''].map(h=>(
                      <th key={h} style={{ padding:'8px 12px', textAlign:'left', color:S.muted, fontWeight:700, fontSize:10, letterSpacing:1, whiteSpace:'nowrap' }}>{h}</th>
                    ))}
                  </tr></thead>
                  <tbody>
                    {filtered.map((l,i)=>(
                      <tr key={l.id} style={{ borderBottom:`1px solid ${S.bord}44`, background:i%2===0?'transparent':S.surf2+'66' }}>
                        ...
                        <td style={{ padding:'10px 12px' }}>
                          <div style={{ fontWeight:700 }}>{l.descricao}</div>
                          {l.obs&&<div style={{ fontSize:10, color:S.muted }}>{l.obs}</div>}
                        </td>
                        ...
                        <td style={{ padding:'10px 12px' }}>
                          <span style={{ background:l.forma_pagamento==='PIX'?'#22C55E22':'#4FC3F722', color:l.forma_pagamento==='PIX'?'#22C55E':'#4FC3F7', borderRadius:4, padding:'2px 8px', fontSize:10, fontWeight:700 }}>
                            {l.forma_pagamento}
                          </span>
                        </td>
                        <td style={{ padding:'10px 12px', textAlign:'center' }}>
                          {l.comprovante_url
                            ? <a href={l.comprovante_url} target="_blank" rel="noreferrer" style={{ fontSize:18, textDecoration:'none' }} title="Ver comprovante">📎</a>
                            : <span style={{ color:S.bord, fontSize:12 }}>—</span>
                          }
                        </td>
                        <td style={{ padding:'10px 12px', fontWeight:800, color:S.gold, textAlign:'right', whiteSpace:'nowrap' }}>{fmt(l.valor)}</td>
                        <td style={{ padding:'10px 6px' }}>
                          <button onClick={()=>excluir(l.id)} style={{ background:'transparent', border:'none', color:'#EF444466', cursor:'pointer', fontSize:14 }}>🗑</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                  <tfoot><tr style={{ borderTop:`2px solid ${S.gold}44` }}>
                    <td colSpan={6} style={{ padding:12, fontWeight:700, color:S.muted, fontSize:12 }}>TOTAL</td>
                    <td style={{ padding:12, fontWeight:900, color:S.gold, fontSize:16, textAlign:'right' }}>{fmt(filtered.reduce((a,b)=>a+b.valor,0))}</td>
                    <td/>
                  </tr></tfoot>
                </table>
              </div>
        ...
        {tab==='cats' && (
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:14 }}>
              {[
                {l:'TOTAL PREVISTO',  v:fmt(totalPrev), c:'#4FC3F7'},
                {l:'TOTAL REALIZADO', v:fmt(totalReal), c:S.gold},
                {l:'SALDO RESTANTE',  v:fmt(saldo),     c:saldo>=0?'#22C55E':'#EF4444'},
              ].map(({l,v,c})=>(
                <div key={l} style={{ ...card, textAlign:'center' }}>
                  <div style={{ fontSize:10, color:S.muted, letterSpacing:2 }}>{l}</div>
                  <div style={{ fontSize:22, fontWeight:900, color:c, marginTop:6 }}>{v}</div>
                </div>
              ))}
            </div>
            {resumo.map(r=>{
              ...
              return (
                <div key={r.categoria} style={{ ...card, borderLeft:`3px solid ${r.cor}` }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:12 }}>
                    <div>
                      <div style={{ fontSize:14, fontWeight:800, marginBottom:2 }}>{r.icone} {r.categoria}</div>
                      <div style={{ fontSize:10, color:S.muted }}>{lc.length} lançamento(s) · {lc.filter(l=>l.comprovante_url).length} com comprovante</div>
                    </div>
                    <div style={{ textAlign:'right' }}>
                      <div style={{ fontSize:18, fontWeight:900, color:r.cor }}>{fmt(r.valor_real)}</div>
                      <div style={{ fontSize:11, color:S.muted }}>de {fmt(r.valor_prev)}</div>
                      <div style={{ fontSize:11, color:r.saldo>=0?'#22C55E':'#EF4444', fontWeight:700 }}>{r.saldo>=0?`Saldo: ${fmt(r.saldo)}`:`⚠️ Estouro: ${fmt(Math.abs(r.saldo))}`}</div>
                    </div>
                  </div>
                  <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:12 }}>
                    <Barra value={r.valor_real} max={r.valor_prev} cor={r.cor}/>
                    <span style={{ fontSize:12, fontWeight:800, color:r.pct_executado>100?'#EF4444':r.cor, whiteSpace:'nowrap' }}>{r.pct_executado}%</span>
                  </div>
                  {lc.length>0
                    ? <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                        {lc.map(l=>(
                          <div key={l.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', background:S.surf2, borderRadius:8, padding:'8px 12px' }}>
                            <div>
                              <span style={{ fontSize:11, fontWeight:700 }}>{l.descricao}</span>
                              <span style={{ fontSize:10, color:S.muted, marginLeft:10 }}>{fmtD(l.data)} · {l.forma_pagamento}</span>
                            </div>
                            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                              {l.comprovante_url && <a href={l.comprovante_url} target="_blank" rel="noreferrer" style={{ fontSize:16, textDecoration:'none' }} title="Ver comprovante">📎</a>}
                              <span style={{ fontSize:13, fontWeight:800, color:S.gold }}>{fmt(l.valor)}</span>
                            </div>
                          </div>
                        ))}
                      </div>
                    : <div style={{ background:S.surf2, borderRadius:8, padding:'10px 16px', fontSize:11, color:S.muted, fontStyle:'italic' }}>Nenhum pagamento registrado ainda</div>
                  }
                </div>
              )
            })}
